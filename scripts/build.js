...
try {
  ...
  execSync('npx esbuild src/cli.ts --bundle --platform=node --outfile=dist/cli.js --external:duck-duck-scrape', { stdio: 'inherit' });
  ...
  // Build the UI only if it exists
  if (fs.existsSync('ui')) {
    ...
    execSync('npx shx cp ui/dist/index.html dist/index.html', { stdio: 'inherit' });
  } else {
